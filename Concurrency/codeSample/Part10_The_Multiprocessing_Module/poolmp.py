...
# -*- coding:utf-8 -*-
# poolmp.py
# An example of using a multiprocessing pool

# A function that does a reasonable amount of processing
import hashlib


# Compute a SHA-512 diges of File
...
TOPDIR = "/Users/mbp-chyi/Dev/github" # Change for your machine
...
def sequential():
    ...
    print("sequential {} seconds".format(end-start))
    return digest_map
    ...
    for path, dirs, files in os.walk(TOPDIR):
        for name in files:
            ...
            if os.path.exists(fullname):
                ...
                digest_map[fullname] = p.apply_async(compute_digest,
                                                     (fullname,))
    # Go through the final dictionary and collect results
    for filename, result in digest_map.items():
        digest_map[filename] = result.get()         # Get the result
    end = time.time()
    print("parallel   {} seconds".format(end-start))
    return digest_map
